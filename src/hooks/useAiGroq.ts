	...
	const generate = useCallback(
		async (prompt: string, systemPrompt?: string) => {
			...
			try {
				...
				// 🛡️ Sentinel: Prevent leaking internal API error details or stack traces to the user in production
				const errorMessage =
					import.meta.env.DEV && err instanceof Error
						? err.message
						: "An error occurred while communicating with the AI service. Please check your API key and try again.";
