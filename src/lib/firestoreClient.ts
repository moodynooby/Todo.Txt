...
export function userDocRef(db: Firestore, uid: string, path: UserDocPath) {
...
export async function getDocWithRetry<T extends DocumentData>(
	...
	await backoff(async () => {
		const snap = await getDoc(userDocRef(db, uid, path));
		if (snap.exists()) {
...
export function writeDocs(
	...
	for (const { path, data } of updates) {
		batch.set(userDocRef(db, uid, path), data, { merge: true });
	}
	return batch.commit();
}
