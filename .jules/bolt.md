...
## 2026-06-02 - [Single-Pass Task Filtering]
**Learning:** Sequential `.filter()` calls on arrays create intermediate garbage and multiple iterations. Combining predicates into a single pass and hoisting invariant computations (like `getToday()` and `toLowerCase()`) significantly improves performance on large task lists.
**Action:** Consolidate multiple array operations into a single loop or pass when possible. Hoist invariant computations out of filter predicates to minimize per-item overhead.
