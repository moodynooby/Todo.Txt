...
## 2025-05-17 - [Consolidated Sidebar Filtering]
**Learning:** Consolidating multiple filter/map operations into a single-pass loop reduces array allocations and redundant iterations. Hoisting predicate factory logic (like date calculations) out of the loop further minimizes per-item overhead.
**Action:** When performing multiple filters or derived calculations on a large list in `useMemo`, use a single `for...of` loop to build the final result and counts in one pass.
